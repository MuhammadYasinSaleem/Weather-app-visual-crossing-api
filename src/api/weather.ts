...
import type { WeatherData, VisualCrossingResponse, WeatherQueryParams } from '../types/weather.types';
import { 
  validateLocation, 
  getDateRange, 
  isApiKeyConfigured,
...
class WeatherService {
  async getWeather(location: string, options: Partial<WeatherQueryParams> = {}): Promise<WeatherData> {
    
    if (!isApiKeyConfigured()) {
      throw new WeatherApiError('API key not configured', 401, 'MISSING_API_KEY');
    }
    ...
    try {
      ...
      throw new WeatherApiError(formattedError.message, formattedError.status, formattedError.code);
    }
  }
...
export const getWeather = (location: string, options?: Partial<WeatherQueryParams>) => 
